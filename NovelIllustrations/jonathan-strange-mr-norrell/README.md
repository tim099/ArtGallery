---
title: 英倫魔法師小說插圖設定集
description: apex-one 依《英倫魔法師》(Jonathan Strange & Mr Norrell) 第一至二章建立的角色、關鍵場景與道具參考；供後續閱讀心得場景圖引用。
author: apex-one (Antigravity)
workflow: AgentCommands/ArtGallery/NOVEL_ILLUSTRATION_WORKFLOW.md
source_media: book-jonathan-strange-mr-norrell
read_through_chapter: "002"
illustrated_through_chapter: "002"
next_chapter: "003"
---

# 《英倫魔法師》小說插圖設定集

## 視覺母題

十九世紀初攝政時期英格蘭北部約克郡的厚重沉靜質感。以冷灰石材、潮濕石階、蜂蠟打磨橡木、哥特拱頂雕花書架與溫暖壁爐火光為基調；第二章導入覆蓋全城的清冽純白大雪與哥特大教堂的巍峨輪廓。色彩以原色小牛皮皮革、深褐毛料、暗紅天鵝絨、純白積雪與陰鬱墨黑交織。設定稿維持經典英國歷史奇幻插畫油畫質地、筆觸細膩、無文字、無水印。

## 範圍與進度

| 項目 | 狀態 |
|---|---|
| 閱讀進度 | 已讀至第 002 章〈古星酒棧〉。 |
| 心得場景插圖 | 已繪至第 002 章。 |
| 下一章 | 第 003 章〈約克的石頭〉；讀完後先摘取新角色／道具，再決定是否需要設定稿。 |
| 劇透邊界 | 嚴格鎖定第一至二章已確認之資訊與外貌設定。 |

## 已鎖定設定

| 類別 | id | 用途 |
|---|---|---|
| 人物 | `mr_norrell` | 實踐派魔法師、何妨寺主人、魔法審查與壟斷者。 |
| 人物 | `john_segundus` | 謙遜真誠的約克求真者，敏銳的觀察主視角。 |
| 人物 | `john_childermass` | 諾瑞爾的司務，陰鬱精明的暗處守門人。 |
| 人物 | `dr_foxcastle` | 約克魔法協會主席，虛榮剛愎的清談權威。 |
| 人物 | `lawyer_robinson` | 體面得體的事務所律師，魔法生死契約的起草者。 |
| 場景/道具 | `hurtfew_abbey_library` | 仿哥特拱頂木雕藏書室，英格蘭魔法典籍的封閉神殿。 |
| 場景/道具 | `york_minster_snow` | 白雪覆蓋的約克大教堂與南門廊，歷史交替的閘門。 |

## 設定資產台帳

| 類別 | id | 設定卡 | 首次關聯章節 | 狀態 |
|---|---|---|---|---|
| 人物 | `mr_norrell` | [吉爾伯特・諾瑞爾](Characters/mr_norrell.md) | 001 | 已繪 v1 |
| 人物 | `john_segundus` | [約翰・斯剛德斯](Characters/john_segundus.md) | 001 | 已繪 v1 |
| 人物 | `john_childermass` | [約翰・齊爾德邁斯](Characters/john_childermass.md) | 001 | 已繪 v1 |
| 人物 | `dr_foxcastle` | [福克斯卡斯爾博士](Characters/dr_foxcastle.md) | 002 | 已繪 v1 |
| 人物 | `lawyer_robinson` | [羅賓森律師](Characters/lawyer_robinson.md) | 002 | 已繪 v1 |
| 場景 | `hurtfew_abbey_library` | [何妨寺藏書室](Props/hurtfew_abbey_library.md) | 001 | 已繪 v1 |
| 場景 | `york_minster_snow` | [雪中約克大教堂](Props/york_minster_snow.md) | 002 | 已繪 v1 |

## 心得場景圖台帳

| 章節 | 展卡 | references | 狀態 |
|---|---|---|---|
| 001 | [實踐派魔法師的宣告](../../ReadingReflections/apex_jonathan_strange_hurtfew_abbey_library.md) | `mr_norrell`, `john_segundus`, `john_childermass`, `hurtfew_abbey_library` | 已繪 |
| 002 | [雪落大教堂前的守門人](../../ReadingReflections/apex_jonathan_strange_york_minster_snow.md) | `john_childermass`, `john_segundus`, `dr_foxcastle`, `york_minster_snow` | 已繪 |

## 待建與尚未鎖定

亨尼福特先生 (Mr. Honeyfoot)、索普先生 (Mr. Thorpe)、普萊森斯太太 (Mrs. Pleasance) 已出場但尚非後續核心主體；第三章即將在約克大教堂展開的奇蹟與石頭低語，將於讀完第三章後按工作流標準建立設定卡。
